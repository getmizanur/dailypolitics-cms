----------------------------------------------------------------------
-- MIGRATION: presentation_styles + post_revisions + new columns
----------------------------------------------------------------------


----------------------------------------------------------------------
-- 1. TABLE: presentation_styles (create if not exists)
--    ✨ UPDATED: Removed background_hex + text_hex
----------------------------------------------------------------------
...
----------------------------------------------------------------------
-- 2. ADD COLUMNS to existing tables (guarded IF NOT EXISTS)
----------------------------------------------------------------------
...
-- 2.2 categories.presentation_style_id
DO $$
BEGIN
  IF to_regclass('public.categories') IS NOT NULL THEN
    ALTER TABLE public.categories
      ADD COLUMN IF NOT EXISTS presentation_style_id INTEGER
        REFERENCES presentation_styles(id) ON DELETE SET NULL;
  END IF;
END;
$$;

-- 2.3 posts.presentation_style_id
DO $$
BEGIN
  IF to_regclass('public.posts') IS NOT NULL THEN
    ALTER TABLE public.posts
...
----------------------------------------------------------------------
-- 4. TRIGGERS for updated_at (only if table + trigger do not exist)
----------------------------------------------------------------------

-- presentation_styles.updated_at trigger
DO $$
BEGIN
  IF EXISTS (
...
-- post_revisions.updated_at trigger
DO $$
BEGIN
  IF EXISTS (
