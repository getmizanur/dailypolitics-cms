    ...
    class RichTextEditor {
        ...
        handleAction(action) {
            ...
            switch(action) {
                ...
                case 'twitter':
                    const twitterUrl = prompt('Enter X/Twitter post URL:');
                    if (twitterUrl) {
                        newText = `[twitter:${twitterUrl}]`;
                        cursorOffset = newText.length;
                    } else {
                        return; // User cancelled
                    }
        ...
        updatePreview() {
            ...
            html = html.replace(/\[twitter:([^\]]+)\]/g, (match, url) => {
                return `<blockquote style="border-left: 4px solid #1d70b8; padding-left: 16px; margin: 16px 0; color: #505a5f;">
                    <p>X/Twitter post: <a href="${url}" target="_blank">${url}</a></p>
                    <p style="font-size: 12px; color: #929292;">(Preview not available in editor)</p>
                </blockquote>`;
            });
